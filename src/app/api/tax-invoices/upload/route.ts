...
const TAX_INVOICE_BUCKET = "tax-invoice-files";
...
export async function POST(request: Request) {
  try {
    ...
    const upload = await supabase.storage.from(TAX_INVOICE_BUCKET).upload(path, file, {
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from(TAX_INVOICE_BUCKET).getPublicUrl(path);
    ...
    return NextResponse.json({ ok: true, invoice: insert.data, contentType: file.type });
  } catch (error) {
    return NextResponse.json(
